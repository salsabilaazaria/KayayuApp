...
class ProfileViewModel {
    ...
    private func getRecurringData () {
        database.collection("recurringTransactions").whereField("user_id", isEqualTo: getUserId()).order(by: "end_billing_date", descending: true).whereField("end_billing_date", isGreaterThan: self.calendarHelper.getEndDay()).getDocuments() { (documentSnapshot, errorMsg) in
            if let errorMsg = errorMsg {
